...
import io.mycat.util.SqlReplace;
...
public abstract class AbstractConnection implements NIOConnection {
    ...
    /**
     * timestamp格式 yyyy-mm-dd hh:MM:ss.fff
     */

    private static final Pattern TIMESTAMP_PATTERN = Pattern.compile("'\\d\\d\\d\\d-\\d\\d-\\d\\d \\d\\d:\\d\\d:\\d\\d(.?\\d*)'");
    private static final Pattern SEQUENCE_PATTERN = Pattern.compile("([a-zA-Z_]+).(nextval)", Pattern.CASE_INSENSITIVE);
...
//    private byte[] handleTimestamp(byte[] data, SqlReplace sqlReplace) {
//        try {
//            String sql = getSql(data);
//            sqlReplace.replaceSql(sql);
//
//            if (flag) {
...
//    }
//
//    private byte[] replaceSequence(byte[] data, SqlReplace sqlReplace) {
//        try {
//            String sql = getSql(data);
//
...
//    }
    ...
    private byte[] replaceSql(byte[] data) {
        SystemConfig system = MycatServer.getInstance().getConfig().getSystem();

        int dataBase = system.getDataBase();
        //0 mysql 1 oracle 2postgresql
        List<SqlReplace> sqlReplaces = new ArrayList<>();
        switch (dataBase) {
            case 1:
                sqlReplaces.add(new SqlReplace(TIMESTAMP_PATTERN, charset) {
                    @Override
                    public String replacement(Matcher matcher) {
                        return "to_timestamp(" + matcher.group() + ",'YYYY-MM-DD HH24:MI:SS.ff')";
                    }
                });
                break;
            case 2:
                sqlReplaces.add(new SqlReplace(SEQUENCE_PATTERN, charset) {
                    @Override
                    public String replacement(Matcher matcher) {
                ...
                sqlReplaces.add(new SqlReplace(WM_CONNECT_PATTERN, charset) {
                    @Override
                    public String replacement(Matcher matcher) {
                        return "string_agg(" + matcher.group(2) + ",',')";
                    }
                });

                sqlReplaces.add(new SqlReplace(REGEXP_PATTERN, charset) {
                    @Override
                    public String replacement(Matcher matcher) {
                        return matcher.group(1) + " like '%" + matcher.group(2) + "%'";
                    }
                });
                break;
            default:
